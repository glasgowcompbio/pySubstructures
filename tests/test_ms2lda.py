...
def test_lda_run(beer_data):
    ...
    assert vlda.n_docs == len(
        lda_dict["corpus"]
    ), "The number of documents should match the corpus size."
    assert vlda.K == 300, "The number of topics (K) should match the predefined value."
    assert vlda.n_words == len(
        lda_dict["word_index"]
    ), "The number of unique words should match the word index size."

    # Ensure all initial topic-document distribution values (alpha) are positive.
    assert all(
        vlda.alpha > 0
    ), "All alpha values must be positive to represent a valid distribution."

    # Confirm the shape of the document-topic distribution matrix.
    assert vlda.gamma_matrix.shape == (
        vlda.n_docs,
        vlda.K,
    ), "The gamma matrix must match the number of documents and topics."
    assert np.all(
        vlda.gamma_matrix >= 0
    ), "All values in the gamma matrix should be non-negative."

    # Validate the shape and normalization of the topic-word distribution matrix.
    assert vlda.beta_matrix.shape == (
        vlda.K,
        vlda.n_words,
    ), "The beta matrix must match the number of topics and unique words."
    # Check that each topic's word distribution sums to 1 for a valid probability distribution.
    assert all(
        np.isclose(vlda.beta_matrix.sum(axis=1), 1)
    ), "Each topic's word distribution should sum to 1."

    assert (
        vlda.n_fixed_topics == 0
    ), "No fixed topics should be present for this basic LDA test."
...
def test_lda_run_with_fixed_motifs(beer_data):
    ...
    vlda = VariationalLDA(
        ...
        K=300,
        normalise=1000.0,
        fixed_topics=motifdb_spectra_dict,
    ...
    assert vlda.n_docs == len(
        lda_dict["corpus"]
    ), "The number of documents must be consistent with the corpus size."
    assert vlda.K == 300 + len(
        motifdb_spectra_dict
    ), "The total number of topics should include both LDA-generated and fixed motifs."
    assert all(
        vlda.alpha > 0
    ), "Alpha values, representing topic-document distributions, must be positive for a valid model."
...
def test_lda_reproducibility(beer_data):
    ...
    # First run
    lda_dict_first_run = msfile_to_corpus(
        beer_data,
        "mzML",
        min_ms1_intensity=0,
        min_ms2_intensity=50,
    ...
    vlda_first_run = VariationalLDA(
        corpus=lda_dict_first_run["corpus"], K=300, normalise=1000.0
    )
    ...
    # Second run
    lda_dict_second_run = msfile_to_corpus(
        beer_data,
        "mzML",
        min_ms1_intensity=0,
        min_ms2_intensity=50,
...
def test_lda_reproducibility_with_fixed_motifs(beer_data):
    ...
    motifdb_spectra_dict = {}
    for motif_id in motifset_ids:
        motifdb_spectra, motifdb_metadata, motifdb_features = acquire_motifdb(
            [motif_id]
        )
    ...
    vlda_first_run = VariationalLDA(
        corpus=lda_dict_first_run["corpus"],
        K=300 + len(motifdb_spectra_dict),
        normalise=1000.0,
        fixed_topics=motifdb_spectra_dict,
        fixed_topics_metadata=motif_metadata,
    )
    ...
    vlda_second_run = VariationalLDA(
        corpus=lda_dict_second_run["corpus"],
        K=300 + len(motifdb_spectra_dict),
        normalise=1000.0,
        fixed_topics=motifdb_spectra_dict,
        fixed_topics_metadata=motif_metadata,
    ...
    assert (
        vlda_first_run.n_fixed_topics == vlda_second_run.n_fixed_topics
