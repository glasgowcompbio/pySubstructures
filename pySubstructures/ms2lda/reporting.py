...
def match_topics_across_dictionaries(
    lda1=None,
    lda2=None,
    file1=None,
    file2=None,
    ...
    if lda1 == None:
        if file1 == None:
            print(
                "Must specify either an lda dictionary object or a dictionary file for lda1"
            )
            return
        else:
        ...
        if same_corpus:
            ...
            for word in lda2["beta"][topic2]:
                ...
                if split_word[0].startswith("fragment"):
                    ppm_errors = 1e6 * np.abs(
                        (fragment_masses - word_mass) / fragment_masses
                    )
    ...
    if summary_file:
        with open(summary_file, "w") as f:
            f.write(
                "lda2_topic,lda1_topic,match_score,unique_words,shared_words,"
    ...
    with open(filename, "w") as f:
        ...
        for doc, doc_pos in vlda.doc_index.items():
            for motif, motif_pos in vlda.topic_index.items():
                if (
                    probs[doc_pos, motif_pos] >= p_thresh
                    and overlap_scores[doc][motif] >= o_thresh
                ):
                    new_row = []
                    new_row.append(doc)
...
def write_topic_report(lda_dictionary, filename, backend="agg"):
    ...
    with PdfPages(filename) as pdf:
        ...
        for topic in tqdm(topic_list):
            ...
            topic_probs = list(
                zip(
                    lda_dictionary["beta"][topic].keys(),
            ...
            for i, (word, prob) in enumerate(topic_probs):
                ...
                textPage.text(
                    ...
                    transform=textPage.transFigure,
                    size=12,
                )
                if rowpos == n_rows - 1:
                    col += 1
...
def write_motifs_in_scans(
    vd, metadata, overlap_thresh, p_thresh, X, motifdb_metadata, output_prefix
):
...
def get_motifs_in_scans(
    lda_dictionary, metadata, overlap_thresh=0.3, p_thresh=0.1, X=5, motif_metadata={}
):
    # write a nodes file
    ...
    for d in lda_dictionary["theta"]:
        for m, p in lda_dictionary["theta"][d].items():
            if (
                p >= p_thresh
                and lda_dictionary["overlap_scores"][d].get(m, 0.0) >= overlap_thresh
            ):
