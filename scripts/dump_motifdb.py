...
def save_motifs(motifdb_spectra, motifdb_metadata, output_dir):
    for motif_name, spectra in motifdb_spectra.items():
        ...
        with open(file_path, "w") as f:
            ...
            f.write(
                "#ANNOTATION {}\n".format(
                    motifdb_metadata[motif_name].get(
            ...
            f.write(
                "#COMMENT {}\n".format(
                    motifdb_metadata[motif_name].get("comment", "No comment available")
                )
            )
...
def download_motifset(base_dir):
    motifset_dict = requests.get(
        MOTIFDB_SERVER_URL + "list_motifsets/", verify=certifi.where()
    ).json()
    ...
    db_list = [
        motif_id
        for motif_id in motifset_dict.values()
        if not whitelist_ids or motif_id in whitelist_ids
    ]

    motif_data = []

    for motif_id in db_list:
        motifdb_spectra, motifdb_metadata, _ = acquire_motifdb(
            [motif_id], filter_threshold=0.95
        )
        motifset_name = motifset_rev_dict[motif_id]
        logger.info(
            f"Downloading {motifset_name} motifset ({len(motifdb_spectra)} motifs)"
        )
    ...
    df = pd.DataFrame(
        motif_data, columns=["Motif ID", "Motif Set", "Number of Motifs"]
    ).set_index("Motif ID")
...
def main():
    parser = argparse.ArgumentParser(
        description="Download all motif sets from MS2LDA and save them to files."
    )
    parser.add_argument(
        "--base_dir",
        type=str,
        default="pySubstructures/resources/MOTIFDB",
        help="Base directory to save the motif sets (default: pySubstructures/resources/MOTIFDB).",
    )

    args = parser.parse_args()

    base_dir = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", args.base_dir)
    )
    ...
    logger.info(
        f"Downloaded and saved all motif sets to '{base_dir}'. See the motif set report above."
    )
